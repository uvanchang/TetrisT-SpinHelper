...
Piece.ghostPiece = function(letter, row, col, rotation) {
  console.log("in ghost constructor");
  var piece;
  switch(letter) {
    case 'O':// O
  ...
  console.log("piece.js row = " + row);
  console.log("piece.js col = " + col);
  piece.row = row;
  piece.column = col;
...
Piece.prototype.canMoveLeft = function(grid){
  ...
  for(var r = 0; r < this.cells.length; r++){
    for(var c = 0; c < this.cells[r].length; c++){
      ...
      if (this.cells[r][c] != 0){
        ...
        if(grid.cells[_r][_c] != 0) {
          if(grid.cells[_r][_c] != 0xD3D3D3) {
            return false;
          } else {
            deleteGhost = true;
          }
  ...
  if(deleteGhost) {
    for(var r = 0; r < grid.cells.length; r++) {
      for(var c = 0; c < grid.cells[0].length; c++) {
